...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(`
${chalk.cyan('⟦ 𝖣𝖮𝖦𝖤 𝖡𝖮𝖳 - 𝖨𝖭𝖥𝖮 ⟧')}
${chalk.cyan('⟶ ⭐')} ${chalk.blue('%s')}
${chalk.cyan('⟶ ⚕️')} ${chalk.cyan('%s')} ${chalk.cyan('%s')}
${chalk.cyan('⟶ 🍭')} ${chalk.magenta('%s [%s %sB]')}
${chalk.cyan('⟶ ✨')} ${chalk.white('%s')}
${chalk.cyan('⟶ 💫')} ${chalk.yellow('%s%s')}
${chalk.cyan('⟶ 🫧')} ${chalk.magenta('%s')} ${chalk.red('%s')}
${chalk.cyan('⟦ 𝖥𝗂𝗇 𝖽𝖾𝗅 𝗂𝗇𝖿𝗈 ⟧')}
`.trim(),
    me + ' ~' + conn.user.name,
  ...
  if (/document/i.test(m.mtype)) console.log(`📄 ${m.msg.fileName || m.msg.displayName || 'Document'}`)
    ...
    console.log(`${m.msg.ptt ? '🎤 (PTT AUDIO DE VOZZ ' : '🎵 ('}AUDIO) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`)
  }
